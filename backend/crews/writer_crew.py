"""WriterCrew — writes structured documentation and guides."""
...
class WriterCrew(BaseFalkensteinCrew):
    ...
    def build_crew(self) -> Crew:
        agent = self._make_agent("writer", self.llm_model, self.fc_llm, self.tools)
        task = Task(
            description=self.task_description,
            expected_output=self.task_configs["writing_task"]["expected_output"],
            agent=agent,
        )
        return Crew(
            agents=[agent],
